import { DomainInfo, domainInfoQuery } from "@anchor-protocol/app-fns/queries/nameservice/domainInfo";
...
export function useNameServiceQuery(addresses: string[] | undefined): UseQueryResult<(DomainInfo | null)[]> {
  ...
  console.log(reverseRecords?.reverseRecords.records)
  ...
  const nameServiceInfo = useQuery(
	...
	async (): Promise<(DomainInfo | null)[]> =>
		pMap(
			...
			async (record: ReverseRecordsItemResponse): Promise<DomainInfo | null> =>{
				console.log(record.record?.token_id)
				return record.record?.token_id
					? domainInfoQuery(queryClient!, contractAddress.nameservice, record.record?.token_id ?? '')
					: null
			}
		),
	{
