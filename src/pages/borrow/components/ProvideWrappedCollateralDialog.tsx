import {
  useAnchorWebapp, useBorrowProvideWrappedCollateralForm,
...
import { ProvideWrappedCollateralFormParams } from './types';
...
function ProvideWrappedCollateralDialogBase(props: ProvideCollateralDialogProps) {
  const {
    className,
    closeDialog,
    txResult,
  ...
  const [input, states] = useBorrowProvideWrappedCollateralForm(
    collateral,
    uTokenBalance,
    fallbackBorrowMarket,
    fallbackBorrowBorrower,
  ...
  useEffect(() => {
    if (!connected || !states.depositAmount || !terraWalletAddress || !collateral.info || !states.lunaAmount) {
      return;
    }
...
export const ProvideWrappedCollateralDialog = styled(ProvideWrappedCollateralDialogBase)`
  width: 720px;
  touch-action: none;
